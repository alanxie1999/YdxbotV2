...
async def _process_bet_on_slim(client, event, user_ctx: UserContext, global_config: dict):
    ...
    # 简单跟随策略：跟随上一手结果下注
    # 检测 6 位纯交替模式（101010 或 010101）
    history = state.history
    if len(history) >= 5:
        last_6 = "".join(str(x) for x in history[-5:])
        if last_6 in ("10101", "01010"):
    ...
    rt["last_predict_confidence"] = 100 if rt.get("last_predict_source") == "alternation_break" else 50
    rt["last_predict_info"] = f"预测方向：{'大' if prediction == 1 else '小'} - {rt['last_predict_reason']}"

    # AI 下注功能已移除，不再执行模型调用和故障处理逻辑
